...
# A2 分档可信度说明（数字固化自 backtest_report.txt；2026-08-09 接入 2024 后重跑回测，
# 覆盖 2024→2025、2025→2026 两对；每次年度接入重跑回测后须同步刷新本段）。
...
CLASSIFICATION_NOTE = {
    ...
    "backtest": {
        "pair": "用某一年数据判档、用次年实际投档门槛检验（同单元跨年对照），"
                "共做两对：2024→2025、2025→2026",
        "margin_coverage": (
            "被判「保」的单元中，次年门槛实际没有越过安全线的比例："
            "2025→2026 物理学科类本科批 91.6%（7,027 个单元）、历史学科类本科批 92.4%（1,896 个单元）；"
            "2024→2025 物理学科类本科批 92.1%（6,858 个单元）、历史学科类本科批 93.3%（1,951 个单元）"
            "——即「保」的判定次年九成以上依旧成立"),
        "rel_delta": (
            "录取位次年际变动：两对回测中，单元变动中位数约 6–8%，九成单元变动在 32% 以内；"
            "变动 ≥50% 的仅 2.9%–5.0%，这类单元会被标为「高波动」"),
    },
