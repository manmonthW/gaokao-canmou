...
## Current Phase
Phase 1.2 — 多维规划与会话重规划：**进行中**
下一步：扩展城市、专业、院校层次和比较任务；增加确定性澄清合同与一次有界 history replan。
...
- [x] 核心 Harness 已提交（5789888）、推送并部署 EWS；Agent/full pytest 128 passed，生产 health/auth/version 验收通过
- **Status:** 已部署完成

### Phase 1.2：多维规划与会话重规划（2026-09-16）
- [x] `TaskSpec` 扩展城市、专业、院校层次、比较目标和澄清字段
- [x] 多值筛选按单一主维度有界 fan-out；其余维度作为每步共同约束，避免笛卡尔爆炸
- [x] 比较请求要求至少两个可验证目标，并在综合提示中显式携带比较合同
- [x] 对“那沈阳呢/改成计算机/比较一下”等短跟进，从最近用户任务恢复基础问题后重新验证和规划
- [x] 无法形成可执行任务时输出针对性追问；最多一次 history replan，不进入自由循环
- [x] 超过 8 个目标不静默截断；`985/211/双一流` 不误作为 `level` 下推，当前明确追问支持范围
- [x] 增加 Harness 与图级回归测试；Agent 71 passed，后端全量 138 passed，`git diff --check` 通过
- **Status:** 本地实现与验证完成，待提交、推送和 EWS 部署验收
