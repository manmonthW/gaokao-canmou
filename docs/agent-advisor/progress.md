...
- `TaskSpec` 新增城市、专业、院校层次、比较输出、澄清和 history replan 字段；模型开放词汇仅在逐字出现于可信输入时可下推。
- 多值任务仅沿一个主维度展开，其他单值约束附加到每个 `search_candidates` 步骤；多个多值维度先追问比较轴。
- 比较缺少两个可验证目标、目标超过 8 个、或请求当前不支持精确下推的 `985/211/双一流` 标签时，在工具调用前定向澄清。
- 短跟进只恢复最近一条可识别的用户任务；“那/再看”扩展目标，“改成/换成”覆盖当前明确维度，不信任 assistant 历史。
- 修正 `plan` 节点条件边使用 `_plan_route`，确保规划阶段产生的澄清不会继续执行工具。
- 版本更新为 graph/toolset/prompts `2026-09-16.3`。
- 验证：Agent 测试 71 passed；后端全量 138 passed；`compileall` 和 `git diff --check` 通过。
- **Status:** 本地实现与验证完成；尚未提交、推送或部署。
