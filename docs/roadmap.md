...
> 下一个迁移编号：**0018**。schema 变更纪律与后续开发注意事项见 `docs/changelog-2026-08-08-d2-d5.md` §六。
...
### ✅ 专业冷热趋势 — 配对键修复 + 锚点调整 + 趋势展示（2026-08-25）

完整记录见 **`docs/changelog-2026-08-25-major-trend.md`**（migration 0017），研究见 `docs/major-trend-2024-2026.md`。

- **P0 修跨年单元配对键**：省内专业代码逐年重排，旧口径按代码归并，本科批 36%/专科批 68% 的多年单元
  把不同专业接成一条时间线。改用规范化专业名后：仅 1 年单元 54%→37%、三年完整单元 +49%、
  高波动 911→70、断档 210→51。同一修复同步 `backtest_match.py`。
- **单调不判高波动**：修好配对后剩余高波动里 64% 是三年单向移动的趋势序列，照常分档由趋势说明解释。
- **A5 稳/冲分界改锚最近一年**：两侧误差同时下降（连降专业浪费槽位 11.2%→8.3%，
  连升专业隐藏风险 4.6%→3.3%）；**保档安全线 best×0.85 不动**；不做趋势外推（回测证明更差）。
  顺带修 `_risk_at` 漏投影 `last_year_rank` 导致断崖提示静默失效。
- **MATCH_CONFIG 重新定参**：8 组回测重跑，margin=0.85 覆盖率由 92.1% 修正为 95.7%~98.5%（本科批），
  **维持 0.85 不放宽**（新数字说明它比设计意图更安全，放宽是唯一可能滑档的方向）。
- **数据层 0017**：`major_trend` 1,544 / `major_trend_alias` 6,353 / `major_trend_market` 8 /
  `major_trend_context` 0（人工撰写，强制带来源与日期）；计算内核 `etl/major_trend_core.py`
  被 ETL 与只读研究脚本共用。顺带修 `major_eval_map` 属主错为只读角色的问题。
- **呈现四面**：匹配页（徽标 + 历年门槛曲线叠加全省大盘基准线 + 趋势详情）、专业查询/抽屉、
  工作台方案体检两条新规则、数据中心方法页 + 全量表。一线数字用**等效分**（超额漂移正号=变松，
  对用户是反的）且必须与大盘并列；「平稳」标签必须同屏带招生单元数变动。
  文案为中性事实（「门槛连降两年」），不用带价值判断的词。顺带修 `.risk-dot` 仅靠颜色的无障碍违规。

验证：pytest 52 passed、golden 契约与 10 个 smoke 全过、迁移与 ETL 幂等、旧库降级 200、前端 build 通过。
下一迁移编号 **0018**。
